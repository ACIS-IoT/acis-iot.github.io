...
#include <fstream>
   ...
   protected:
     ...
     * Transmit a message
     */
    void Robot::transmit(int H, int pld)
    {
        ...
        send(TRANSMIT, "Robot_TRANSMIT"); // send out the message
    }
     ...
     */
    std::vector<std::string> Robot::extractSubstrings(const std::string& inputString) {
    ...
    void Robot::writeDataToCSV(const std::string& action, const std::string& source, int h, int pld) {
