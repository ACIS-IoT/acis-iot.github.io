...
#include <fstream>
   ...
   protected:
    ...
    void WV::handleMessage(cMessage *msg)
    {
     ...
     * Transmit a message
     */
    void WV::transmit(int H, int pld)
    {
        ...
        send(TRANSMIT, "WV_TRANSMIT"); // send out the message
    }
     ...
     */
    std::vector<std::string> WV::extractSubstrings(const std::string& inputString) {
    ...
    void WV::writeDataToCSV(const std::string& action, const std::string& source, int h, int pld) {
