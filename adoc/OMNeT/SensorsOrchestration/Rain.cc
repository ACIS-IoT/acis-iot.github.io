...
#include <fstream>
     ...
     */
    class Rain : public cSimpleModule
    {
    public:
        Rain();
    private:
   ...
   protected:
    ...
    void Rain::handleMessage(cMessage *msg)
    {
     ...
     * Transmit a message
     */
    void Rain::transmit(int H, int pld)
    {
        ...
        send(TRANSMIT, "Rain_TRANSMIT"); // send out the message
    }
     ...
     */
    std::vector<std::string> Rain::extractSubstrings(const std::string& inputString) {
    ...
    void Rain::writeDataToCSV(const std::string& action, const std::string& source, int h, int pld) {
