...
#include <array>
    ...
    private:
        ...
        void restart();
        void writeDataToCSV(int D1H, int D2H, int D3H, int SYNCH);
   protected:
    ...
    void InBox::handleMessage(cMessage *msg)
    {
        ...
        if(isFound){
            ...
            if(gateIndex==0){
                ...
                if(gateIndex==2){
                               ...
                               full++;
                                    ...
                                    int collectedclk = std::stoi(extractedSubstrings.first );
                                    int collectedpld= std::stoi(extractedSubstrings.second );
                                    timeArray[2]= collectedclk;
                                    pldArray[2]= collectedpld;
                                    full++;
                           }
            }
            ...
            if(full==ARRAY_SIZE){
                int rclk= (timeArray[0]+timeArray[1]+timeArray[2])/3;
                broadcast(rclk, pldArray[1]);
                full=0;
                writeDataToCSV(timeArray[0],timeArray[1],timeArray[2],rclk);
            }
        }
    ...
    void InBox::broadcast(int H, int pld){
        ...
        send(LISTEN, "Rain_LISTEN");
        sendDelayed(LISTEN->dup(), 1e-9,"WL_LISTEN");
        sendDelayed(LISTEN->dup(), 1e-3,"WV_LISTEN");
    }
    /**
     * Reschedule the send
     */
    ...
    void InBox::writeDataToCSV(int D1H, int D2H, int D3H, int SYNCH) {

        std::string result = std::string(simTime().str()) + ","+ std::string(std::to_string(D1H))+"," +std::string(std::to_string(D2H))+","+std::string(std::to_string(D3H))+","  + std::string(std::to_string(SYNCH));
