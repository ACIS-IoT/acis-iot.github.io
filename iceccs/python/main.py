...
df[["RP", "WV", "WH", variable, target]] = df[["RP", "WV", "WH", variable, target]].astype(int)

# Prepare the features and target variables
X = df[["RP", "WV", "WH", variable ]]
y = df[target]
...
def traverse_tree(node_id=0, depth=0, parent_probability=1.0, at=0):
    ...
    if tree.feature[node_id] != -2:  # If not a leaf node
        ...
        #print(f" if no AT:")
        traverse_tree(tree.children_left[node_id], depth + 1, parent_probability * tree.weighted_n_node_samples[node_id] / tree.weighted_n_node_samples[0], at=0)
        #print(f"{indent}else:")
